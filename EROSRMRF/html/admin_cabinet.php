...
while($row = $res->fetch_assoc()) {
    echo "
         ...
         ";
            ...
            echo "
           <br><br><h4 class=\"page-title text-center\">Управление аккаунтами</h4><br> 
           <div class=\"row\">                        
                                <div class='col-md-12 '>
...
if (!$res=$mysqli->query(" Call CountPost()")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
}
$countPost = $res->fetch_assoc();
$countPost=$countPost['count'];

$mysqli = ConnectDB();
if (!$res=$mysqli->query(" Call CountAllLike()")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
}
$countAllLike = $res->fetch_assoc();
$countAllLike=$countAllLike['countAllLike'];

$mysqli = ConnectDB();
if (!$res=$mysqli->query(" Call CountAllDislike()")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
}
$countAllDislike = $res->fetch_assoc();
$countAllDislike=$countAllDislike['countAllDislike'];

$mysqli = ConnectDB();
if (!$res=$mysqli->query(" Call CountAllLikeExpert()")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
}
$countAllExpertLike = $res->fetch_assoc();
$countAllExpertLike=$countAllExpertLike['count'];

$mysqli = ConnectDB();
if (!$res=$mysqli->query(" Call CountAllDislikeExpert()")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
}
$countAllExpertDislike = $res->fetch_assoc();
$countAllExpertDislike=$countAllExpertDislike['count'];
       ...
       echo "<div class='col-md-6'>
               ...
               <div class=\"row\"> 
                <div class='col-md-12'>
                                       <ul class=\"list-group\">
                                        <li class=\"list-group-item\">Количество постов $countPost</li>
                                        <li class=\"list-group-item\"> Количество лайков $countAllLike </li>
                                        <li class=\"list-group-item\"> Количество дизлайков $countAllDislike</li>
                                        
                                        <li class=\"list-group-item\"> Количество лайков $countAllExpertLike </li>
                                        <li class=\"list-group-item\"> Количество дизлайков $countAllExpertDislike</li>
                                    </ul>
               </div>
               </div>
              ...
              <br><br><h4 class=\"page-title text-center\">Управление экспертами</h4><br> 
           <div class=\"row\">                        
                                <div class='col-md-12 '>
